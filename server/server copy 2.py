...
def send_signal_to_clients(endpoint, payload):
    ...
    for url, status, response in responses:
        print(f"Client: {url}, Status: {status}, Response: {response}")
...
def listen_for_signals():
    ...
    while True:
        ...
        if input_command == 'start':
            # 이미 실행 중인 쓰레드가 없을 때만 시작
            if task_thread is None or not task_thread.is_alive():
                stop_requested.clear()
                print("Sending start signal to all clients...")
                # 별도의 쓰레드에서 실행 (콘솔 응답 유지)
                task_thread = threading.Thread(target=send_signal_to_clients, args=('start_signal', {"start": True}))
                task_thread.start()
            else:
            ...
            stop_requested.set()
            print("Sending stop signal to all clients...")
            send_signal_to_clients('stop_signal', {"stop": True})
            
            # 작업 쓰레드가 종료되도록 대기
            if task_thread is not None:
                task_thread.join()
                task_thread = None  # 쓰레드 초기화
